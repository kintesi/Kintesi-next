      ...
      >
            ...
            >
              {slide.bgImage ? (
                <>
                  ...
                  />
                  {/* Luxury dark & gradient overlay for crystal clear text readability */}
                  <div className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/70 to-black/60" />
                  <div className={`absolute inset-0 bg-gradient-to-t ${getFlashThemeClasses(theme)} opacity-40 mix-blend-overlay`} />
                </>
              ) : (
        ...
        <div className="relative z-10 p-4 space-y-2.5">
          ...
          <div className="flex items-center justify-between gap-2">
            {/* Tag Badge */}
            <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-white/20 backdrop-blur-md text-[10px] font-black uppercase tracking-wider border border-white/15">
              <Flame className="w-3 h-3 fill-white text-amber-300 animate-pulse" />
              <span>{activeSlides[safeIndex]?.tag || '⚡ FLASH SALE'}</span>
            </div>

            {/* Timer */}
            <div className="flex items-center gap-1 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-lg border border-white/25 text-[10px] font-mono font-bold shadow-xs">
          ...
          <div key={safeIndex} className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            <h3 className="text-sm font-black text-white leading-tight drop-shadow-sm line-clamp-2">
              {activeSlides[safeIndex]?.title}
            </h3>
            {activeSlides[safeIndex]?.subtitle && (
              <p className="text-white/85 text-[10px] mt-0.5 leading-relaxed drop-shadow line-clamp-2">
                {activeSlides[safeIndex]?.subtitle}
              </p>
            )}
          </div>
    ...
    >
          ...
          >
            {slide.bgImage ? (
              <>
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-gray-950/90 via-gray-950/70 to-black/40" />
                <div className={`absolute inset-0 bg-gradient-to-t ${getFlashThemeClasses(theme)} opacity-35 mix-blend-overlay`} />
              </>
            ) : (
              <div className={`w-full h-full bg-gradient-to-r ${getFlashThemeClasses(theme)}`} />
            )}
      ...
      {activeSlides.length > 1 && (
        <>
          <button
            type="button"
            onClick={handlePrev}
            className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/40 hover:bg-black/70 backdrop-blur-md border border-white/20 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 active:scale-90 cursor-pointer"
          ...
          <button
            type="button"
            onClick={handleNext}
            className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/40 hover:bg-black/70 backdrop-blur-md border border-white/20 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 active:scale-90 cursor-pointer"
      ...
      <div className="relative z-10 p-8 flex items-center justify-between gap-6">
        <div className="space-y-2 max-w-2xl">
          {/* Tag Badge */}
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-xs font-black uppercase tracking-wider border border-white/15 shadow-xs">
          ...
          <div key={safeIndex} className="animate-in fade-in slide-in-from-bottom-3 duration-600">
            ...
            {activeSlides[safeIndex]?.subtitle && (
              <p className="text-white/85 text-xs mt-1 leading-relaxed max-w-xl drop-shadow">
                {activeSlides[safeIndex]?.subtitle}
              </p>
            )}
          </div>
              ...
              {activeSlides.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${
                    i === safeIndex ? 'w-7 bg-amber-400 shadow-sm shadow-amber-400/50' : 'w-2 bg-white/35 hover:bg-white/60'
                  }`}
                  aria-label={`Go to slide ${i + 1}`}
                />
              ))}
              <span className="text-[10px] font-bold text-white/60 ml-1.5">
                {safeIndex + 1} / {activeSlides.length}
              </span>
            </div>
          )}
        ...
        <div className="flex items-center gap-2 bg-black/50 backdrop-blur-md p-3 rounded-2xl border border-white/20 shadow-xl shrink-0">
