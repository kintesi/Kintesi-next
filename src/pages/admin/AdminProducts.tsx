...
import { getProductsFromDB, saveProductToDB, deleteProductFromDB, getCategoriesFromDB, getProductBySlugOrId } from '../../lib/dbService';
...
export const AdminProducts: React.FC = () => {
  ...
  const [viewingDescProduct, setViewingDescProduct] = useState<Product | null>(null);
  const [descViewMode, setDescViewMode] = useState<'formatted' | 'raw'>('formatted');
  const [editDescTab, setEditDescTab] = useState<'edit' | 'preview'>('edit');
  ...
  const handleOpenEditModal = (prod: Product) => {
    ...
    if (prod.id) {
      getProductBySlugOrId(prod.id).then((fullProd) => {
        if (fullProd && fullProd.description) {
          setFormData((prev) => ({
            ...prev,
            description: fullProd.description || prev.description,
            warranty: fullProd.warranty || prev.warranty,
            fabric: fullProd.fabric || prev.fabric,
            fit_type: fullProd.fit_type || prev.fit_type,
            care_instructions: fullProd.care_instructions || prev.care_instructions,
            origin: fullProd.origin || prev.origin,
            gender: fullProd.gender || prev.gender,
          }));
        }
      }).catch(() => {});
    }
  ...
  return (
    <div className="w-full space-y-6 pb-20">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="max-w-xs">
                            ...
                            {/* Description Preview & Full Viewer Button */}
                              ...
                              {prod.description && prod.description.trim() ? (
                                <div className="space-y-1">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setViewingDescProduct(prod);
                                      setDescViewMode('formatted');
                                    }}
                                    className="text-[10px] font-bold text-rose-600 dark:text-rose-400 hover:underline inline-flex items-center gap-1 cursor-pointer"
                                  >
                                    <FileText className="w-3 h-3" />
                                    <span>ডেসক্রিপশন দেখুন (View Full)</span>
                                  </button>
                                </div>
                              ) : (
                                <div className="flex items-center gap-1 text-[10px] text-amber-600 dark:text-amber-400 font-semibold bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 w-fit">
                                  <AlertTriangle className="w-3 h-3" />
                                  <span>কোনো ডেসক্রিপশন নেই</span>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className={`p-2 rounded-xl transition cursor-pointer ${
                              isLight ? 'hover:bg-slate-100 text-cyan-600' : 'hover:bg-gray-800 text-cyan-400'
                            }`}
                            title="প্রোডাক্টের ডেসক্রিপশন দেখুন (View Description)"
                          >
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-xs font-bold text-gray-300 flex items-center gap-2">
                      <span>Detailed Description (পণ্যের সম্পূর্ণ বিবরণ)</span>
                      <span className="text-[10px] font-normal text-gray-400">
                        ({formData.description.length} অক্ষর • {formData.description.trim().split(/\s+/).filter(Boolean).length} শব্দ)
                      </span>
                    </label>
                    <div className="flex items-center gap-1 bg-gray-900 p-0.5 rounded-lg border border-gray-800">
                      <button
                        type="button"
                        onClick={() => setEditDescTab('edit')}
                  ...
                  {editDescTab === 'edit' ? (
                    <textarea
                      ...
                      className="w-full bg-gray-900 border border-gray-700 rounded-xl p-3.5 text-xs text-white focus:outline-none focus:border-emerald-500 leading-relaxed font-sans"
                    />
                  ) : (
                    <div className="bg-white text-gray-900 p-5 rounded-2xl border border-gray-300 max-h-96 overflow-y-auto shadow-inner">
                      {formData.description && formData.description.trim() ? (
                        <FormattedProductDescription description={formData.description} />
                      ) : (
                        <p className="text-gray-400 text-xs text-center py-8">কোনো ডেসক্রিপশন লেখা হয়নি।</p>
                      )}
                    </div>
                  )}
                </div>
      ...
      {/* Quick Full Description Viewer Modal */}
      {viewingDescProduct && createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-3 sm:p-6 bg-black/70 backdrop-blur-xs animate-fadeIn">
          <div
            className={`w-full max-w-3xl rounded-3xl border shadow-2xl flex flex-col max-h-[90vh] overflow-hidden ${
              isLight ? 'bg-white border-slate-200 text-slate-900' : 'bg-gray-900 border-gray-800 text-white'
            }`}
          >
            {/* Header */}
            <div className={`p-4 sm:p-5 border-b flex items-center justify-between gap-3 ${
              ...
              <div className="flex items-center gap-3 min-w-0">
                <img
                  src={viewingDescProduct.images?.[0] || '/logo.webp'}
                  alt=""
                  className="w-10 h-10 rounded-xl object-cover border border-gray-700/50 shrink-0 bg-gray-950"
                />
                <div className="min-w-0">
                  <h3 className="text-sm sm:text-base font-black truncate">
                    {viewingDescProduct.title}
                  </h3>
                  <div className="flex items-center gap-2 text-[11px] text-gray-500 mt-0.5">
                    <span>SKU: {viewingDescProduct.sku || 'N/A'}</span>
                    <span>•</span>
                    <span>{(viewingDescProduct.description || '').length} অক্ষর</span>
                    <span>•</span>
                    <span>{(viewingDescProduct.description || '').trim().split(/\s+/).filter(Boolean).length} শব্দ</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className={`flex items-center p-0.5 rounded-xl border ${
                  isLight ? 'bg-slate-200/70 border-slate-300' : 'bg-gray-800 border-gray-700'
                }`}>
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  onClick={() => setViewingDescProduct(null)}
                  className={`p-2 rounded-full transition cursor-pointer ${
                    isLight ? 'hover:bg-slate-200 text-slate-500' : 'hover:bg-gray-800 text-gray-400'
                  }`}
                >
                  <X className="w-5 h-5" />
                </button>
            ...
            <div className="p-5 sm:p-8 overflow-y-auto flex-1">
              {viewingDescProduct.description && viewingDescProduct.description.trim() ? (
                descViewMode === 'formatted' ? (
                  <FormattedProductDescription description={viewingDescProduct.description} />
                ) : (
                  <pre className={`text-xs sm:text-sm font-mono whitespace-pre-wrap leading-relaxed p-4 rounded-2xl border ${
                    isLight ? 'bg-slate-50 border-slate-200 text-slate-800' : 'bg-gray-950 border-gray-800 text-gray-200'
                  }`}>
                    {viewingDescProduct.description}
                  </pre>
                )
              ) : (
                <div className="py-12 text-center space-y-3">
                  ...
                  <p className="text-sm font-bold">এই প্রোডাক্টে এখনো কোনো ডেসক্রিপশন যুক্ত করা হয়নি।</p>
                </div>
              )}
            </div>
            ...
            <div className={`p-4 border-t flex items-center justify-between gap-3 ${
              ...
              <button
                type="button"
                onClick={() => {
                  if (viewingDescProduct.description) {
                    navigator.clipboard.writeText(viewingDescProduct.description);
                    toast.success('ডেসক্রিপশন টেক্সট কপি হয়েছে!');
                  }
                }}
                disabled={!viewingDescProduct.description}
                className={`px-4 py-2 rounded-xl text-xs font-bold border transition flex items-center gap-1.5 cursor-pointer disabled:opacity-40 ${
                  isLight ? 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100' : 'bg-gray-800 border-gray-700 text-gray-200 hover:bg-gray-700'
                }`}
              >
                <Copy className="w-3.5 h-3.5" />
                <span>কপি করুন (Copy Text)</span>
              </button>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const prodToEdit = viewingDescProduct;
                    setViewingDescProduct(null);
                    handleOpenEditModal(prodToEdit);
                    setActiveModalTab('specs');
                  }}
                  className="px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs transition flex items-center gap-1.5 cursor-pointer shadow-md shadow-rose-600/30"
                >
                  <Edit2 className="w-3.5 h-3.5" />
                  <span>স্টুডিওতে এডিট করুন (Edit in Studio)</span>
                </button>
              </div>
            </div>
          </div>
