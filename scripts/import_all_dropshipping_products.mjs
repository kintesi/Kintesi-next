...
async function main() {
  ...
  for (let i = 0; i < allRawProducts.length; i++) {
    ...
    let detectedBrand = 'No Brand';
    const descMatch = (raw.details || '').match(/(?:brand|ব্র্যান্ড|Brand Name)\s*[:：\-–]\s*([^\r\n,<|.]+)/i);
    if (descMatch) {
    ...
    if (detectedBrand === 'No Brand') {
      for (const b of KNOWN_BRANDS_LIST) {
        const escaped = b.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
        const regex = new RegExp('(?:^|[\\s\\[\\(\\/\\-_])' + escaped + '(?:$|[\\s\\]\\)\\/\\-_:,])', 'i');
        if (regex.test(raw.name)) {
          if (b.toLowerCase() === 'vision' && /night\s+vision|clear\s+vision/i.test(raw.name)) continue;
          if (b.toLowerCase() === 'nova' && /super\s+nova/i.test(raw.name)) continue;
          detectedBrand = b;
          break;
        }
      }
