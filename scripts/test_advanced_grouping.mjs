...
const COLOR_MAP = {
  ...
  nevi: { name: 'Navy', hex: '#1E3A8A', bn: 'নেভি' },
...
for (const p of catalog) {
  ...
  // Normalized key: keeps English, Bengali, digits
...
console.log(`Unique master groups after advanced normalization: ${groups.size}`);

const multiVariantGroups = Array.from(groups.values()).filter(g => g.items.length > 1);
console.log(`Groups with multiple variants / duplicates consolidated: ${multiVariantGroups.length}`);

let totalMergedCount = 0;
multiVariantGroups.forEach(g => totalMergedCount += g.items.length);
console.log(`Total items merged into multi-variant groups: ${totalMergedCount}`);
