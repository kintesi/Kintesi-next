...
function extractSizes(p) {
  ...
  if (sizes.length === 0 && Array.isArray(p.custom_attributes)) {
    const sizeAttrs = p.custom_attributes.filter(a => a.attribute && a.attribute.toLowerCase().includes('size') && a.name);
    if (sizeAttrs.length > 0) {
      sizes = Array.from(new Set(sizeAttrs.map(a => a.name.trim())));
    }
  }

  if (sizes.length === 0 && p.description) {
    const descMatch = p.description.match(/Size\s*:\s*([^\n\r]+)/i);
    if (descMatch && descMatch[1]) {
      const raw = descMatch[1].replace(/M\s*=\s*Chest.*$/i, '').trim();
      const tokens = raw.split(/[\s,+/|]+/).map(s => s.trim().toUpperCase()).filter(s => /^(M|L|XL|XXL|XXXL|4XL|5XL|S|28|30|32|34|36|38|40|42|44|46|48|50|52|54|56)$/.test(s));
      if (tokens.length > 0) {
        sizes = Array.from(new Set(tokens));
      }
    }
  }
...
function generateAccurateTags(title, categoryId, subCategory, brand) {
  ...
  if (brand && brand !== 'Generic' && brand !== 'Kintesi') tagSet.add(brand.toLowerCase());
...
async function run() {
  console.log('Classifying and extracting sizes for all', catalog.length, 'products...');

  let sizeExtractedCount = 0;
  let reclassifiedCount = 0;

  const updatedCatalog = catalog.map((prod) => {
    const classification = smartClassify(prod.title, prod.category_id, prod.brand);
    if (classification.category_id !== prod.category_id || classification.sub_category !== prod.sub_category) {
      reclassifiedCount++;
    }

    const sizes = extractSizes(prod);
    if (sizes.length > 0 && (!prod.sizes || prod.sizes.length === 0)) {
      sizeExtractedCount++;
    }

    const specMode =
      classification.category_id.includes('fashion')
        ? 'fashion'
        : classification.category_id.includes('gadget') || classification.category_id === 'electronic-accessories' || classification.category_id === 'computer-gaming'
        ? 'gadgets'
        : 'auto';
  ...
  console.log(`Updated ${reclassifiedCount} product categories.`);
  console.log(`Restored sizes for ${sizeExtractedCount} products.`);
  ...
  console.log('Syncing all 2,829 products to Supabase...');
  ...
  for (let i = 0; i < updatedCatalog.length; i += BATCH_SIZE) {
    ...
    if (error) {
      console.error(`\nError syncing batch ${i} - ${i + chunk.length}:`, error.message);
    } else {
  ...
  console.log('\nAll products successfully reclassified with sizes and synced to Supabase!');
}
